...
//  
static void gotosleepopen(gpio_num_t pin){
  esp_sleep_enable_ext0_wakeup(pin, 0);		// 1== HIGH (Sensor Open)
...
static void gotosleepclose(gpio_num_t pin){
...
static void timer_cb(void *arg)
{
    ...
    printf(strBuffer);
    mgos_mqtt_pub(mgos_sys_config_get_app_mqtt_tank_level_topic(), strBuffer, strlen(strBuffer), 1, 0);
    printf("Door is open, going to sleep.\n");
    gotosleepopen(GPIO_PIN);
  }
...
enum mgos_app_init_result mgos_app_init(void)
{
    ...
    mgos_gpio_set_mode(25, MGOS_GPIO_MODE_INPUT);
    mgos_gpio_set_pull(25, MGOS_GPIO_PULL_UP);
