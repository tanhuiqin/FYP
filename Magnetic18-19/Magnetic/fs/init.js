...
load('api_config.js'); //imported for sample reporting metrics to AWS IoT
load('api_mqtt.js');   //imported for sample reporting metrics to AWS IoT
load('api_sys.js');   //imported for sample reporting metrics to AWS IoT
load('api_timer.js');   //imported for sample reporting metrics to AWS IoT


GPIO.set_mode(23, GPIO.MODE_INPUT);
GPIO.set_pull(23, GPIO.PULL_UP);
...
let topic = 'iot/cubicalDoor' + Cfg.get('device.id');

Timer.set(4000 /* milliseconds */, Timer.REPEAT, function() {
  let msg = JSON.stringify({status2:GPIO.read(23),date:Timer.now(),sensorID:'1'});
  print(topic, '->', msg);
  MQTT.pub(topic, msg, 0);
}, null);

let f = ffi('bool mgos_ota_commit()');
    f(); //fixes the update commit timeout expire issue
